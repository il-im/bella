...
import { fetchHit, handleGotoDetails } from '../../redux/actions/hit_action'
import { postFavourite } from  '../../redux/actions/favourite_action'
import { Link } from "react-router-dom";


const TopSales = () => {
    const hits = useSelector((state) => state.hitReducer.hits)  
    // console.log(hits , 'topSale + Hits');  
    ...
    return hits.map((item, index) => {
        const addToCartHandler = (e) => {
            ...
            // console.log(item, 'topsales');
        
        return (
            <div className={classes.wrapper} key={item.id} index={index}>
                    <Link to="/details">
                        <button onClick={() => {dispatch(handleGotoDetails(item))}}>Go to details</button>
                    </Link>
                <div className={classes.main_cart_wrap} >
                    <img src={dress_model} alt="blue-dress" />
                    <div className={classes.favourite} 
                            onClick={() => dispatch(postFavourite(item.id))}
                    >
                    ...
                    <div className={classes.cart_wrapper_main}>
                        <div className={classes.cart_wrapper}>
                            ...
                            <span onClick={(e) => addToCartHandler()}>Добавить корзину</span>
                        </div>
                    </div>
