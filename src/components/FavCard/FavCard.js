...
import classes from "./FavCard.module.scss";
...
const FavCard = (prod) => {
    ...
    // console.log(prod.prod.id, 'prodddd');
    const dispatch = useDispatch()
    return (
        <>
        <div className={classes.wrapper} key={prod.prod.id} >
            <div className={classes.main_cart_wrap}>
                {/* {prod.prod.images.map((item, i) => { */}
                    <img src={dress_model} alt="blue-dress" />
                {/* })} */}
                <div className={classes.favourite}
                    onClick={() => dispatch(postFavourite(prod.prod.id))}
                >
                ...
                <div className={classes.product_description}>
                    <div className={classes.description_text}>
                        <span>{prod.prod.title}</span>
                    </div>

                    <div className={classes.size_description}>
                    {prod.prod.size.map((item, i) => {
                        return (
                            <span key={item + 3}>{item.size}</span>
                        )
                    })}
                    </div>
                </div>
            ...
            <div className={classes.color_wrapper}>
                {prod.prod.color.map((color, index) => {
                    return (
                        <div
